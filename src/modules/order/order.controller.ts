...
export const createOrder = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!Array.isArray(items) || items.length === 0) return next(new AppError("Order items are required", 400));

    const orderItems: any[] = [];

    for (const item of items) {
        if (!item.type || !item.id) return next(new AppError("Each item must have type and id", 400));
        ...
        switch (item.type) {
            case "DIGITAL_PRODUCT": {
                ...
                if (!product) return next(new AppError("Digital product not found", 404));
                if (!product.isActive) return next(new AppError("Digital product is not available", 400));
            ...
            case "DIGITAL_ACCESS": {
                ...
                if (!access.isActive) return next(new AppError("Digital access is not available", 400));
            ...
            case "DIGITAL_GROWTH": {
                ...
                if (!growth.isActive) return next(new AppError("Digital growth service is not available", 400));
...
export const cancelOrder = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!order) return next(new AppError("Order not found", 404));
    if (order.status !== "PENDING") return next(new AppError("Only pending orders can be cancelled", 400));
