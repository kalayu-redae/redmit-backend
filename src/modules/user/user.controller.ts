...
const DEFAULT_AVATAR = (req: Request) => `${req.protocol}://${req.get("host")}/uploads/defaults/default-avatar.png`;

const userResponse = (req: Request, user: any) => ({ id: user.id, email: user.email, phone: user.phone, username: user.username, fullName: user.fullName, avatarUrl: user.avatar?.url || DEFAULT_AVATAR(req), role: user.role, isVerified: user.isVerified, isActive: user.isActive, createdAt: user.createdAt, updatedAt: user.updatedAt });
...
const buildUserQuery = (query: any) => {
    ...
    const where: any = {};
    if (email) where.email = { contains: String(email).trim(), mode: "insensitive" };
    if (fullName) where.fullName = { contains: String(fullName).trim(), mode: "insensitive" };
    if (phone) where.phone = { contains: String(phone).trim(), mode: "insensitive" };
    if (username) where.username = { contains: String(username).trim(), mode: "insensitive" };
    if (role) where.role = role;
    if (isVerified !== undefined) where.isVerified = isVerified === "true";
    if (isActive !== undefined) where.isActive = isActive === "true";
    if (joinDate || leaveDate) {
        where.createdAt = {};
        if (joinDate) where.createdAt.gte = new Date(String(joinDate));
        if (leaveDate) where.createdAt.lte = new Date(String(leaveDate));
    }
    if (search) {
...
export const getUsers = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!allowedSortFields.includes(sortBy)) return next(new AppError("Invalid sort field", 400));
...
export const updateUser = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (normalizedEmail !== undefined && normalizedEmail !== existingUser.email) {
        ...
        if (emailExists) return next(new AppError("Email is already registered", 409));
    }
    if (normalizedPhone !== undefined && normalizedPhone !== existingUser.phone) {
        ...
        if (phoneExists) return next(new AppError("Phone number is already registered", 409));
    }
    if (normalizedUsername !== undefined && normalizedUsername !== existingUser.username) {
        ...
        if (usernameExists) return next(new AppError("Username is already taken", 409));
    }
    const data: any = {};
    ...
    if (fullName !== undefined) data.fullName = fullName === null ? null : String(fullName).trim();
...
export const deleteUsers = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { ids } = req.body;
    if (!Array.isArray(ids) || !ids.length) return next(new AppError("User IDs are required", 400));
...
export const deleteAllUsers = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const sendEmailToUser = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params as { id: string };
...
export const sendEmails = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const sendEmailToAllUsers = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { subject, message } = req.body;
    if (!subject || !message) return next(new AppError("Subject and message are required", 400));
    const users = await prisma.user.findMany({ where: { isActive: true }, select: { email: true } });
    if (!users.length) return next(new AppError("No active users found", 404));
    await Promise.all(users.map(user => sendEmail({ email: user.email, subject, message })));
