...
export const createPayment = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!method) return next(new AppError("Payment method is required", 400));
    ...
    if (method === "MANUAL_BANK_TRANSFER") {
        if (!bankAccountId) return next(new AppError("Bank account is required", 400));

        const bankAccount = await prisma.bankAccount.findFirst({
            where: { id: bankAccountId, isActive: true },
        });

        if (!bankAccount) return next(new AppError("Bank account not found or inactive", 404));
    }
...
export const verifyPayment = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!payment) return next(new AppError("Payment not found", 404));
    if (payment.method !== "MANUAL_BANK_TRANSFER") return next(new AppError("Only manual payments require manual verification", 400));
    if (payment.manualStatus === "VERIFIED") return next(new AppError("Payment is already verified", 400));
...
export const rejectPayment = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params as { id: string };
    ...
    if (!payment) return next(new AppError("Payment not found", 404));
    if (payment.status === "COMPLETED") return next(new AppError("Completed payment cannot be rejected", 400));
