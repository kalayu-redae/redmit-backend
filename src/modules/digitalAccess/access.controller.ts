...
export const createDigitalAccess = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (existingAccess) return next(new AppError("Access slug already exists", 409));
...
export const getDigitalAccesses = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const getDigitalAccess = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!access) return next(new AppError("Digital access not found", 404));

    return res.status(200).json({ status: 1, message: "Digital access retrieved successfully", data: access });
});

export const getMyDigitalAccesses = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const providerId = req.user?.id;
    if (!providerId) return next(new AppError("Authentication required", 401));
...
export const updateDigitalAccess = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params as { id: string };
    const providerId = req.user?.id;
    ...
    if (!access) return next(new AppError("Digital access not found", 404));
    if (access.providerId !== providerId) return next(new AppError("You are not allowed to update this access", 403));
...
export const updateDigitalAccessStatus = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params as { id: string };
    const providerId = req.user?.id;
...
export const deleteDigitalAccess = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params as { id: string };
    const providerId = req.user?.id;
    ...
    const access = await prisma.digitalAccess.findUnique({ where: { id } });
    if (!access) return next(new AppError("Digital access not found", 404));
    if (access.providerId !== providerId) return next(new AppError("You are not allowed to delete this access", 403));
