...
import { OpportunityType } from "../../generated/prisma/enums.js";
...
export const getOpportunities = catchAsync(
    async (req: Request, res: Response) => {
        ...
        if (type) where.type = type as OpportunityType;
...
// ── GET ONE ────────────────────────────────────────────────────────────────────
export const getOpportunity = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
...
export const updateOpportunity = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!existing) return next(new AppError("Opportunity not found", 404));
        ...
        if (slug !== undefined) {
            ...
            if (newSlug !== existing.slug) {
                ...
                if (slugExists) return next(new AppError("An opportunity with this slug already exists", 409));
            }
        }
...
// ── PUBLISH ────────────────────────────────────────────────────────────────────
export const publishOpportunity = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
        const id = String(req.params.id);
...
// ── UNPUBLISH ──────────────────────────────────────────────────────────────────
export const unpublishOpportunity = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
        const id = String(req.params.id);
