...
export const createProduct = catchAsync(async (req, res, next) => {
    ...
    if (!name || !categoryId || price === undefined)
        return next(new AppError("Name, category and price are required", 400));
...
export const getProducts = catchAsync(async (req, res, next) => {
    ...
    const where = {};
    if (search)
        where.OR = [{ name: { contains: String(search), mode: "insensitive" } }, { description: { contains: String(search), mode: "insensitive" } }, { shortDescription: { contains: String(search), mode: "insensitive" } }];
    if (categoryId)
    ...
    if (minPrice !== undefined || maxPrice !== undefined)
        where.price = { ...(minPrice !== undefined && { gte: Number(minPrice) }), ...(maxPrice !== undefined && { lte: Number(maxPrice) }) };
...
export const deleteProduct = catchAsync(async (req, res, next) => {
    ...
    if (product.thumbnailId)
        fileIds.push(product.thumbnailId);
