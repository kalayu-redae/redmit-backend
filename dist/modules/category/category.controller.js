...
export const updateCategory = catchAsync(async (req, res, next) => {
    ...
    if (name !== undefined || newSlug !== undefined) {
        const duplicate = await prisma.category.findFirst({ where: { id: { not: id }, OR: [...(name ? [{ name: { equals: name.trim(), mode: "insensitive" } }] : []), ...(newSlug ? [{ slug: newSlug }] : [])] } });
        if (duplicate)
            return next(new AppError("Category name or slug already exists", 409));
    }
    ...
    if (req.file) {
        ...
        imageId = newFile.id;
        if (category.imageId)
            await FileManager.delete(category.imageId);
    }
...
export const deleteCategory = catchAsync(async (req, res, next) => {
    ...
    if (productCount > 0)
        return next(new AppError("Cannot delete category because it contains products", 400));
