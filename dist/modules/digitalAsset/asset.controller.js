...
const buildSlug = (value) => value.trim().toLowerCase().replace(/\s+/g, "-");
export const createDigitalAsset = catchAsync(async (req, res, next) => {
    ...
    if (existingAsset)
        return next(new AppError("Asset slug already exists", 409));
...
export const getDigitalAssets = catchAsync(async (req, res, next) => {
    ...
    if (isSold !== undefined)
        where.isSold = String(isSold) === "true";
...
export const getDigitalAsset = catchAsync(async (req, res, next) => {
    const { id } = req.params;
...
export const getMyDigitalAssets = catchAsync(async (req, res, next) => {
    const sellerId = req.user?.id;
    if (!sellerId)
        return next(new AppError("Authentication required", 401));
...
export const updateDigitalAsset = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const sellerId = req.user?.id;
    if (!sellerId)
        return next(new AppError("Authentication required", 401));
    ...
    if (asset.sellerId !== sellerId)
        return next(new AppError("You are not allowed to update this asset", 403));
    const { name, slug, type, description, price, currency, platform, username, profileUrl, followers, following, posts, views, country, niche, engagementRate, monthlyRevenue, revenueCurrency, category, url, otherPlatform, details } = req.body;
    const files = req.files;
    const thumbnailFile = files?.thumbnail?.[0];
    const assetFiles = files?.files || [];
    ...
    if (finalType === "SOCIAL_MEDIA_ACCOUNT") {
        if (asset.otherDetails)
            await prisma.otherAsset.delete({ where: { assetId: id } });
    ...
    if (finalType === "OTHER") {
        if (asset.socialMediaDetails)
            await prisma.socialMediaAsset.delete({ where: { assetId: id } });
...
export const updateDigitalAssetStatus = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const sellerId = req.user?.id;
    if (!sellerId)
        return next(new AppError("Authentication required", 401));
...
export const markDigitalAssetAsSold = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const sellerId = req.user?.id;
    if (!sellerId)
        return next(new AppError("Authentication required", 401));
...
export const deleteDigitalAsset = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const sellerId = req.user?.id;
    if (!sellerId)
        return next(new AppError("Authentication required", 401));
