...
export const createDigitalGrowth = catchAsync(async (req, res, next) => {
    ...
    if (existingGrowth)
        return next(new AppError("Growth service slug already exists", 409));
...
export const getDigitalGrowths = catchAsync(async (req, res, next) => {
...
export const getDigitalGrowth = catchAsync(async (req, res, next) => {
    const { id } = req.params;
...
export const getMyDigitalGrowths = catchAsync(async (req, res, next) => {
    const providerId = req.user?.id;
    if (!providerId)
        return next(new AppError("Authentication required", 401));
...
export const updateDigitalGrowth = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const providerId = req.user?.id;
    if (!providerId)
        return next(new AppError("Authentication required", 401));
    ...
    if (!growth)
        return next(new AppError("Digital growth service not found", 404));
    if (growth.providerId !== providerId)
        return next(new AppError("You are not allowed to update this service", 403));
    const { name, slug, type, description, price, currency, details } = req.body;
    const files = getUploadedFiles(req);
    const thumbnailFile = files?.thumbnail?.[0];
    const growthFiles = files?.files || [];
...
export const updateDigitalGrowthStatus = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const providerId = req.user?.id;
    if (!providerId)
        return next(new AppError("Authentication required", 401));
...
export const deleteDigitalGrowth = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const providerId = req.user?.id;
    if (!providerId)
        return next(new AppError("Authentication required", 401));
    ...
    if (!growth)
        return next(new AppError("Digital growth service not found", 404));
    if (growth.providerId !== providerId)
        return next(new AppError("You are not allowed to delete this service", 403));
